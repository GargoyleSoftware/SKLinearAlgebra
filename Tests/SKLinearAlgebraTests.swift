...
class SKLinearAlgebraTests: XCTestCase {
    ...
    func testangleBetweenVectors() {
        ...
        XCTAssertEqualWithAccuracy(angle(a, right: a), Float(0), accuracy: EPSILON_LESS, "Same vector is parallel")
        XCTAssertEqualWithAccuracy(angle(a, right: b), Float(0), accuracy: EPSILON_LESS, "Parallel vectors are parallel")
        XCTAssertEqualWithAccuracy(angle(a, right: c), Float(M_PI), accuracy: EPSILON_LESS, "Opposite vectors are opposite")

        let p = SCNVector3(x: 4, y: 0, z: 7)
        let q = SCNVector3(x: -2, y: 1, z: 3)

        XCTAssertEqualWithAccuracy(angle(p, right: q), Float(1.1252), accuracy: EPSILON_LESS, "Random vectors have correct angle.")
    }
    ...
    func testVectorComponent() {
        ...
        XCTAssertEqual(component(q, b: n), result, "component test")

        let a = SCNVector3(x: 1, y: 2, z: 3)
        let b = SCNVector3(x: 1, y: 2, z: 3)

        XCTAssertEqualWithAccuracy(component(a, b: b), magnitude(a), accuracy: EPSILON, "equal vectors component returns magnitude")

        let c = SCNVector3(x: -1, y: -2, z: -3)

        XCTAssertEqualWithAccuracy(component(a, b: b), magnitude(a), accuracy: EPSILON, "opposite vectors component returns magnitude")

        let d = SCNVector3(x: 0, y: 0, z: 2)
        let e = SCNVector3(x: 1, y: 0, z: 0)

        XCTAssertEqual(component(d, b: e), Float(0), "orthogonal vectors component is zero")
    }
